/*
	Copyright (C) 2009, 2011 Petri Tuononen
...
public class BlocksOf3Padding {
	...
	private String getDecodecBlockFormula(int encoded) {
		...
		if (division < BASE) { //two products
			...
			sB.append(encoded+" = "+division+times+BASE+"\n");
			sB.append(encoded+" = "+secondProduct+times+BASE+" + "+dPart+times+BASE+"\n");
			sB.append(encoded+" = "+secondProduct+times+BASE+" + "+lastProduct+times+BASE+pow_zero+"\n");
		}
		else { //three products
			//result after the first division is >26 so another division has to be done
			sB.append(encoded+" = ("+encoded+"/"+BASE+")"+times+BASE+"\n");
